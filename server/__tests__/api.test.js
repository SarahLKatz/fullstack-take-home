...
describe('Course endpoints', () => {
  ...
  it('/courses gets all courses', async (done) => {
    ...
    expect(firstCourse.description).toEqual("The author is trash but the world is magical")
    done()
 })
 ...
 it('/courses/:id/sessions gets all session for a given course', async (done) => {
  ...
  expect(sessions[0].description).toEqual("Basilisks are scary, but Harry is brave.")
  done()
 })
})